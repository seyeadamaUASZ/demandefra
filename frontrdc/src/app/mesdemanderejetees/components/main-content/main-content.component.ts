...
import { MesdemanderejeteesService } from '../../service/mesdemanderejetees.service';
...
export class MainContentComponent implements OnInit {
	...
	constructor(private mesdemanderejeteesService: MesdemanderejeteesService,
	...
	ngOnInit() {
		this.listeMesDemandesRejetees();
	}

	listeMesDemandesRejetees() {
		this.userService.getUserByUsername(localStorage.getItem('username')).subscribe((data: any) => {
			this.mesdemanderejeteesService.getMesDemandesRejetees(data.data.utiEmail).subscribe((data: any) => {
				if (data.statut) {
